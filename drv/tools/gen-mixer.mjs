...
function emitGate() {
  ...
  return [
    ...
    "        ; DUE, and A still holds the byte that said so. BUSY is bit 7 of it",
    "        ; and it is worth WAITING for: the combined `(a & $82) == $02` test",
    "        ; an earlier revision used read \"busy\" as \"not due\" and RETURNED —",
    "        ; and the call sites that keep the YM busiest are the slot's own",
    "        ; write loops, the longest stretches a frame has. Measured: a",
    "        ; recurring 2,700-cycle hole in cr_p0 where the timer had been due",
    "        ; the whole time and every ask was answered no. So the two are",
    "        ; asked separately, but out of ONE read.",
    "        rla                    ; carry = BUSY",
    "        jr   nc,et_go",
    "et_busy:",
    "        ld   a,(YM_ADDR0)",
    ...
    "        ; THE WRAP, ahead of the read rather than behind the advance.",
    "        ;",
    ...
    "        ;",
    "        ; Wrapping BEFORE the read instead of after the advance is what lets",
    "        ; it sit here, in the gap behind a chip write, and costs the same",
    "        ; either way: the cursor is only ever read while it is valid.",
    "        ld   a,iyh",
    "        cp   RING_TOP>>8",
    "        jr   nz,et_ok",
    "        ld   iy,RING_BUF",
