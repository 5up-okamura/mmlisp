...
function emitGate() {
  return [
    ...
    "        ;",
    "        ; THE PORTS LIVE IN THE SHADOW PAIR. HL' = YM_ADDR0 and DE' = YM_DATA0",
    "        ; are constants, written once at boot, and `ld (hl),a` is 7 cycles",
    "        ; against `ld (nn),a`'s 13 — four of those a sample, less the 8 the",
    "        ; `exx` pair costs, is 16 cycles off an emit that sits inside ~100%",
    "        ; of the stretches that run past a sample period.",
    "        ;",
    "        ; REENTRANCY: this runs both inside the interrupt (which never",
    "        ; enables them) and in the idle loop (which does), so a vblank can",
    "        ; land between the two `exx` and leave the handler in the wrong bank.",
    "        ; `di` around it does not work — the window is ~150 cycles and the",
    "        ; VDP holds /INT for 171.5, which is too close. The handler's",
    "        ; prologue normalises the bank instead, WITHOUT knowing which one is",
    "        ; live (engine.z80, int_handler).",
    "        exx",
    "        ld   a,$27",
    "        ld   (hl),a            ; [1/4] the flag-reset register's address",
    ...
    "        ld   a,(G_FILL+1)",
    "        or   a",
    "        jr   z,et_dry          ; the ring is empty — nothing finished to send",
    "        dec  a",
    "        ld   (G_FILL+1),a",
    "        ld   a,255",
    "        ld   (G_FILL),a",
    ...
    "et_nw:",
    "et_none:",
    "        exx                    ; …and the caller's registers come back",
    "        ret",
    "; THE RING HAD NOTHING. Counted, because a hole in the DAC's output has two",
    "; causes with opposite fixes — nobody asked inside the period, or somebody",
    "; asked and there was nothing to send — and every number this branch has had",
    "; reports them together (engine.z80, H_DRY). Off the send path entirely, so",
    "; it costs the sample clock nothing.",
    "et_dry:",
    "        ld   a,(H_DRY)",
    "        inc  a",
    "        ld   (H_DRY),a",
...
;
; RATE-STAMP ${PCM_BAKE_STAMP} ${PCM_RING_TARGET} — see tools/rate-mirrors.mjs. This
; file's emit shape follows PCM_GROUP, so it is rate-dependent like the rest.
;
...
// ask-dense.z80: one `call feed_one`, or nothing at all.
//
// THE ASK IS NOT FREE and its value depends on the RATE. An ask that finds the
// timer not due costs 43 cycles; what it buys is splitting a stretch that would
// otherwise have run past a sample period and thrown an overflow away. Below
// ASK_DENSE_CY the period is short enough that the stretches this file sits in
// straddle one; above it they do not, and the 43 cycles are pure loss.
//
// MEASURED, four scores, 300 frames, delivered share of the clock:
//
//   period   rate      without these two          with them
//    1075   3329 Hz
//     806   4439    98.7 98.3 98.2 97.4      97.0 97.9 98.2 97.0   worse
...
// does, from the same constant.
export const SAMPLE_PERIOD_CY = Math.round(59736 / PCM_SAMPLES_PER_FRAME);
export const ASK_DENSE = SAMPLE_PERIOD_CY < ASK_DENSE_CY;

export const ASKDENSE_PATH =
  join(dirname(fileURLToPath(import.meta.url)), "..", "src", "ask-dense.z80");

export function askDenseSource() {
  return `; GENERATED (tools/gen-mixer.mjs, askDenseSource) — DO NOT EDIT BY HAND.
;
; Included at the two places a pass transition runs longest without asking:
; after mvf_ringcap on the sounding path and on the silent one. At a short
; sample period they are worth their 43 cycles and at a long one they are not,
; so the sample clock decides and not this file. See ASK_DENSE_CY.
;
; The sample period here is ${SAMPLE_PERIOD_CY} Z80 cycles against a threshold of ${ASK_DENSE_CY}.
;
; RATE-STAMP ${PCM_BAKE_STAMP} ${PCM_RING_TARGET} — see tools/rate-mirrors.mjs.
${ASK_DENSE ? "        call feed_one" : "; (nothing: the period is long enough that these stretches fit inside one)"}
`;
}
...
;
; RATE-STAMP ${PCM_BAKE_STAMP} ${PCM_RING_TARGET}
...
export function generatedSources() {
  return { [MIXER_PATH]: mixerSource(), [RATE_PATH]: rateSource(),
           [ASKDENSE_PATH]: askDenseSource() };
}

export function writeRate() {
  writeFileSync(RATE_PATH, rateSource());
  writeFileSync(ASKDENSE_PATH, askDenseSource());
  return RATE_PATH;
}
