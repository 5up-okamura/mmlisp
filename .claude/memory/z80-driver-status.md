...
| Rare-event handlers resident | **25 B** (d_marker only) | tempo set/sweep, CSM, FM3 mode evicted to ovl_rare (step 7). d_marker stays resident — **but the marker gate now exists** (2026-07-18, `(trig N)` work: verify.mjs diffs MB_TSTAT id bits Z80≡drv-player per frame, `m3-trig` + every existing label marker, 0-diff), so **d_marker eviction to ovl_rare is now verifiable = ~25 B funding unblocked**. |
