...
- **Engine** — three pair-transport Z80 engine images, one per PCM voice count
  ...
  from its own instruction stream, generated by `drv/engine/`.
- **Host** — `drv/sgdk/` (two pumps a frame or VBlank-only, render-ahead
  ...
  (`drv/tests/sin008.mmlisp`) played on BlastEm with every write and DAC byte
  graded on the one-voice engine that preceded the three images; the images
  are graded in the JS machine and move to BlastEm with the host's next step.
