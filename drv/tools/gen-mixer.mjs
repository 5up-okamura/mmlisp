...
function emitGate() {
  return [
    ...
    "        rla",
    "        jr   c,et_busy",
    "        ; …and the RING must have a finished sample. This is the whole of",
    "        ; the accounting now: no per-frame debt, no reserve, no flush. The",
    "        ; mixer adds what it wrote to G_FILL and every send takes one back,",
    "        ; which is what a ring IS — and it is what lets the send happen from",
    "        ; anywhere, including after the interrupt has returned.",
    "        ;",
    ...
    "        jr   c,et_b2           ; four writes back to back — tolerable at",
    "                               ; 3.5 gates a frame, which is what it was",
    "                               ; written for. Per-sample gating makes it 55,",
    "                               ; so the frame's YM traffic went from 14",
    "                               ; writes to 220 and the chip is being written",
