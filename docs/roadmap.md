...
- **Format and reference** — MMB v0.3 and the opcode/target tables frozen
...
- **Sequencer** — `drv/68k/mmlispseq.c`, byte-identical to `drv-player.js` on
  52 scores (`npm run c-gate`): notes, loops, CALL/RET, tempo, sweeps, macros,
...
- **Engine** — three pair-transport Z80 engine images, one per PCM voice count
  ...
  from its own instruction stream, generated by `drv/engine/`. A score picks
  its image with `(def pcm-voices N)`; loop points move while a note plays.
...
- [x] Pitch-to-rate mapping: `rate = 2^(semitones_from_C4 / 12)`, baked per note; no range clamp
...
- **PCM (part K)**: the `#pcm` bank (`*pcm.bin`, up to 32 YM2608 ADPCM-B
  ...
  `@n` (1-based) rebinds the track's sample. The WAV keeps mucom's own 16 kHz;
  the MMB exporter bakes each note from it at the engine image's rate. A bank also imports standalone, as
