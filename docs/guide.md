...
(def-voice lead init-fm :alg 4 :fb 3 :tl1 30)
...
Samples are defined with `(def-sample name …)`, then named in a `pcm1` / `pcm2` / `pcm3` body — before the notes, and again wherever the sound changes — as a voice is on FM.
...
```lisp
(def-sample snare :file "sounds/snare.wav"
...
(def-sample snare-hot snare :effect [(fade :len 60ms)])
```
...
```lisp
(def-voice fm-init :alg 0 :fb 0 :ams 0 :fms 0
