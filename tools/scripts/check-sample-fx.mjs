...
// rejects `:effect` forms, an import's chain and an extending `(def-sample name base …)` compose,
...
(def-sample s :file "/x.wav" :effect [(gain 3) (comp :ratio 2 :attack 2ms) (fade :len 8)])
(pcm1 s :tempo 120 :len 4 c)`, "t.mmlisp");
  ...
  check("compiler resolves the chain",
    ...
    JSON.stringify(fx));
  const instant = compileMMLisp(`(def-sample s :file "/x.wav" :effect [(comp :attack 0ms) (fade :at 0ms :len 5ms)])`, "t.mmlisp");
  ...
  for (const [keys, code] of cases) {
    const { diagnostics } = compileMMLisp(`(def-sample s :file "/x.wav" ${keys})`, "t.mmlisp");
...
(def-sample s :file "/x.wav" ${effect})
(pcm1 s :tempo 120 :len 4 c)`, "t.mmlisp");
...
{
  const kit = `(def-sample kick :file "wav/kick.wav" :frames 900 :effect [(gain -3)])
(def-sample snare :file "wav/snare.wav")
(def-sample snare-kit snare :effect [(crush 6)])`;
  const { ir, diagnostics } = compileMMLisp(`(def pcm-voices 1)
(import "kit/set.mmlisp" :effect [(comp) (limit)])
(def-sample kick-short kick :frames 400)
(def-sample snare-hot snare :effect [(gain 3)])
(def-sample snare-own snare :file "mine.wav")
(def-voice lead init-fm :alg 4)
...
(fm1 lead c)`, "t.mmlisp", { imports: new Map([["kit/set.mmlisp", kit]]) });
  ...
  check("def-sample with a base inherits the file and both chains",
    by("kick-short")?.resolvedFile === "kit/wav/kick.wav" && chain("kick-short") === "comp limit gain",
    `${by("kick-short")?.resolvedFile} / ${chain("kick-short")}`);
  check("def-sample with a base overrides what it writes", by("kick-short")?.frames === 400 && chain("snare-hot") === "comp limit gain",
    `${by("kick-short")?.frames} / ${chain("snare-hot")}`);
  check("def-sample with a base: its own :file reads from its own folder", by("snare-own")?.resolvedFile === "mine.wav",
