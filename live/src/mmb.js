...
// the page, drv/tools/pairs-model.mjs levelPage). vel and vol compose into the voice's
// shift; master is a shift of its own that the host adds when it picks the
// page, so it is common to every voice at no cost to the engine.
//
// How far vel + vol may attenuate a voice. Past 4 the sample keeps 3 bits or
// fewer — quantisation noise, not a quieter sample — so the level CLAMPS here
// rather than mutes: vel alone never silences a voice.
// Mirrored by MML_PCM_MAX_SHIFT in drv/68k/mmlispseq.h (the c-gate diffs the
// two slot streams, so they cannot drift apart silently).
export const PCM_MAX_SHIFT = 4;

// Master's own ceiling: 6 steps, -36 dB. `master 12..1` share the bottom rung.
export const PCM_MASTER_MAX_SHIFT = 6;

// Voice shift + master shift at which the voice is MUTED rather than played:
// 7 leaves a single bit of the sample. With `vol 0` and `master 0` it is one of
// the three hard mutes.
