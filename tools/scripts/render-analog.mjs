#!/usr/bin/env node
// Offline single-channel YM2612 render + ideal waveform references, analog waveform comparison.
...
const out=nodePath.join(os.tmpdir(),'mmlisp-renders','analog')+nodePath.sep;fs.mkdirSync(out,{recursive:true});
const kinds=[['wave-sine','sine'],['wave-triangle','triangle'],['wave-saw','saw'],['wave-square',.5],['wave-pulse-25',.25]];
...
for(const [name,kind] of kinds){
 ...
 for(const note of [48,60,72]){
  ...
  for(let i=0;i<frames;i++){
   ...
   // Continuous ideal waveforms for reference; these are not chip oscillators.
   let value=typeof kind==='number'?(phase<kind?1:-1):kind==='sine'?Math.sin(2*Math.PI*phase):kind==='triangle'?1-4*Math.abs(phase-.5):2*phase-1;
...
const audition=fs.readFileSync(root+'examples/source/analog-audition.mmlisp','utf8');
const compiled=compileMMLisp(audition,'analog-audition.mmlisp',{imports:new Map([['../../presets/waveforms/waveforms.mmlisp',source]])});
assert.deepEqual(compiled.diagnostics,[]);assert.deepEqual(encodeMmb(compiled.ir).diagnostics,[]);
fs.writeFileSync(root+'presets/waveforms/render-report.json',JSON.stringify({nativeRate:rate,wavRate:Math.round(rate),notes:[48,60,72],noteSeconds:.65,restSeconds:.2,preview:'DC removed and RMS matched to 0.15; peak capped at 0.95. Direct register render at base TL, equivalent to full velocity/volume; no runtime macros.',voices:report},null,2)+'\n');
