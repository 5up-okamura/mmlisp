...
9. `(a b c)` — subgroup: divide current `:len` equally among elements (Bresenham); `_` supported
10. `c`, `d+`, `b-`, etc. — bare note name; plays at current octave with current length and gate
