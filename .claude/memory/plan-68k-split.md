...
## THE DRIFT — 2026-08-31. FIXED (confirmed by ear on the user's own conversion).
...
Two changes did it, and they are different faults with the same symptom:
...
**A CLAIM I MADE AND THE USER'S EARS DISPROVED.** I argued that any lost sample
is permanent drift — Timer B's flag being one bit — and therefore that the bar
is 100.0% and 99.1% must still drift. It does not. The reasoning was too
loose: a missed send is not automatically a stretch of the content, because the
ring's cursor and its clamp decide whether the loss shows up as a hold or as a
drop, and the mixer's production stays frame-locked either way. Do not use
"98-99% is not good enough" as a rule; measure the symptom.

**What is still true and still worth watching, on drv/tests/sin008.mmlisp:**
...
2. The ring runs EMPTY 1.09 times a frame. A one-frame lead leaves no room for
   the dip inside a frame. The lead cannot simply be raised: it IS the offset,
   and raising it means raising `mml_render_frame()`'s compensation to match —
   sequencer surgery, not a constant. Neither is audible today.
...
**1. feed_one tail-calls emit_send.** It was spilling HL around a routine
that does not touch HL — emit_send decrements the fill through A alone for
exactly that reason, and says so — and returning through its own `ret`.
38 cycles. `jp emit_send`.

**2. The emit's YM ports live in HL'/DE'.** Constants written once at boot;
four `ld (hl),a`/`ld (de),a` at 7 replace four `ld (nn),a` at 13, less 8 for
the `exx` pair. The shadow set has been free since the mixer stopped
resampling.

THE REENTRANCY, which is why this was abandoned once before and is now
solved. The emit runs inside the interrupt (which never enables them) and
in the idle loop (which does), so a vblank can land between the two `exx`
and leave the handler in the wrong bank — its emit would then write the
sample through the mixer's HL. `di` does not work: the window is ~150
cycles against a 171.5-cycle /INT pulse. The prologue normalises the bank
...
Whichever bank is live, `exx` puts the other in reach, the constants go
there, `exx` puts it back — so the bank the emit's own `exx` reaches holds
the ports in both cases. It costs the idle loop's HL/DE, which are dead
there, and can leave a permanent parity flip, which costs nothing because
the mixer loads its file from RAM. `frame-budget` gates it: 400 frames of
sin008 at PCM_FM=8 enter the handler mid-emit 118 times with 0 stray
writes; remove the four instructions and it is 27 stray and 877% of the
clock (the engine writing over itself).

**3. Two more asks, GENERATED by the rate.** `src/ask-dense.z80` is either
one `call feed_one` or a comment, included at the two places a pass
transition runs longest without asking. An ask that finds the timer not
due costs 43 cycles and only pays where the stretch it splits would have
straddled an overflow, so the threshold is the sample period:
ASK_DENSE_CY = 600 (the only number here that was chosen, not measured).

    period  rate       without                    with
     806  4439 Hz  98.7 98.3 98.2 97.4     97.0 97.9 98.2 97.0   worse
     672  5327     97.6 98.1 98.2 96.4     98.0 97.7 97.8 96.7   a wash
     538  6658     96.4 96.7 97.4 96.5     96.6 97.5 97.1 96.1   a wash
     403  8878     93.6 93.0 93.8 91.9     94.6 93.5 94.7 93.2   better

A THIRD ask nearly shipped a real bug: **feed_one destroys the flags**, and
it went between `call pp_voice_at` and the `jr nz` that reads its answer.
Not a crash — the mixer runs the wrong pass — and only two of four scores
moved far enough to look wrong (93.6/93.0/93.8/91.9 -> 87.6/68.5/93.9/50.8).
`build-engine` now refuses a build where a flag-conditional immediately
follows `call feed_one`, in engine.z80 or any generated source.

**4. The lead regulator is centred on what the DAC really takes.** THIS IS
THE ONE THAT MADE 8,878 Hz USABLE. The proportional controller leaves a
steady-state error, and at a short period that error is the whole problem:
the engine sends ~93% of what the clock owes, equilibrium is production =
consumption, and the loop reaches it by holding

    err = GAIN x (take - want) = 8 x (138 - 148) = -80

samples of lead that never come in — measured as fill 245 against a target
of 149, the DAC 10.8 ms behind the FM, which is exactly the offset that
made a snare hit twice. So `want` is now MEASURED: production minus
consumption is the change in fill, so over 24 frames the consumption is
(what was mixed) + (the fill then) - (the fill now), exact in the average
however deep mml_pump's lookahead runs.

NOT the integral term that was tried and rejected here. That one pushed
past balance and shrank the chunk, and the chunk is also the emit
schedule, so it cost eight points of delivered samples. Centring changes
what the correction is measured FROM; at balance `aim` is the same number
either way and only the fill it settles at moves.

    BlastEm, 8 s, the ring's lead:      8878 Hz          6658 Hz
      m3-pcm-softmix              +10.8 ms -> +0.1   -0.5 -> -0.3
      sin008                       +1.1    -> +1.0   +1.2 -> -0.9

WHERE THE RATES STAND (four scores, 300 frames, delivered share):

    PCM_FM   12     10      8      6        5
    Hz     4439   5327   6658   8878    10653
          98.7   97.6   96.6   94.6     90.5
          98.3   98.1   97.5   93.5     88.9
          98.2   98.2   97.1   94.7     90.9
          97.4   96.4   96.1   93.2     85.9

8,878 Hz is now the working ceiling for 3 voices: ~94% delivered with the
lead pinned inside a millisecond. 10,653 is not — 86-91% and falling
fast.

WHAT IS LEFT, AND IT IS ONE NUMBER. Every remaining over-period stretch is
~1.1-1.4 periods and ~100% of them have an emit in them. The emit is
~180 cycles, which is 45% of a 403-cycle period, and the work between
asks cannot go below ~250. The two remaining fats:

* the fill's 16-bit decrement, 46 cycles. BC' would make it 22 — and it
  CANNOT be done the way the ports were. The ports work because they are
  CONSTANTS the handler can re-establish blind; the fill is STATE, and a
  handler entered mid-emit runs in the bank holding it and destroys it.
* the master chain's `jr` at shift 0, 12 cycles. Removing it means folding
  master into the mixer's ring write, which delays master changes by the
  lead. Not worth 12.

So one-sample-per-overflow gating is at its floor around 9 kHz. Past that
the model has to change to XGM2's: a timer gating a GROUP of samples with
the emits inside the group placed by cycle counting, which is what
PCM_GROUP > 1 was and it has never worked on this branch (build-engine
refuses it by name). That is the next fork, and it is a design decision,
not an optimisation.
