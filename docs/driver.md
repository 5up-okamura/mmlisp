...
> floor, so the reference/asm build places **all** RAM data above the code at
> `DATA_BASE` (currently 0x18D0; code owns 0x0000–0x18CF). From there: mailbox
> 0x18D0, val slots 0x1910, driver globals 0x1930, channel state 0x19A0, TCB
> 0x1C20, shadow 0x1E20. The shadow is a 2×152 B value plane plus a **bit**-per-
> register valid plane (2×19 B) for change-only suppression — the byte-per-
> register valid plane the earlier draft used was dropped to fit 8 KB. The
> mailbox and val slots are the only 68k-published addresses; they move with
> `DATA_BASE`, so `drv/sgdk/mmlispdrv.c` uses the current values. `DATA_BASE`
> bumps as the image grows (re-checking the top fits under the stack). See
> `drv/README.md`.

Constant tables inside the code image (all generated by the JS reference,
§12): FM F-number LUT 24 B, PSG period LUT 144 B, level tables 96 B (§7),
PCM rate multipliers 98 B (M2), curve unit LUTs 4 × 256 B (M2). Total
≈ 1.4KB of the 4.5KB code budget.
