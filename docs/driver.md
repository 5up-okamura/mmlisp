...
> floor, so the reference/asm build places **all** RAM data above the code:
> `DATA_BASE = 0x1780` (code owns 0x0000–0x177F). From there: mailbox 0x1780,
> val slots 0x17C0, driver globals 0x17E0, channel state 0x1880, TCB 0x1B00,
> shadow (value + valid planes, 2×152 B each) 0x1D00. The mailbox and val slots
> are the only 68k-published addresses; they moved with the floor, so
> `drv/sgdk/mmlispdrv.c` uses the current values. `DATA_BASE` bumps as the image
> grows (re-checking the top fits under the stack). See `drv/README.md`.
